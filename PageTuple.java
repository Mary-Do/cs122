...
public abstract class PageTuple implements Tuple {
    ...
    private void setNullColumnValue(int iCol) {
        ...
        /* If the value is already NULL, don't do anything. */
        if (isNullValue(iCol)) {
            return;
        }

        /* Otherwise, the column's flag in the tuple's null-bitmap must be set 
         * to true. */
        setNullFlag(iCol, true);

        /* Determine the column's type. */
        ColumnType columnType = schema.getColumnInfo(iCol).getType();

        /* Get storage size of the column. */
        int bytesToBeDeleted;
        if (columnType.getBaseType() == SQLDataType.VARCHAR) {
            bytesToBeDeleted = getStorageSize(columnType, columnType.getLength());
        } 
        else {
            bytesToBeDeleted = getStorageSize(columnType, 0);
        }
        logger.debug("Bytes to be deleted: " + bytesToBeDeleted + 
            " at location " + valueOffsets[iCol]);

        /* DELETE */
        deleteTupleDataRange(valueOffsets[iCol], bytesToBeDeleted);

        /* Update valueOffsets to indicate that the column value is now NULL. */
        valueOffsets[iCol] = NULL_OFFSET;
    ...
    private void setNonNullColumnValue(int iCol, Object value) {
        ...
        if (isNullValue(iCol)) {
            ...
            logger.debug("Used to be NULL");
        }

        int oldOffset = valueOffsets[iCol];

        /* Figure out the size of the old column-value. */
        ColumnType columnType = schema.getColumnInfo(iCol).getType();
        int old_size = getColumnValueSize(columnType, valueOffsets[iCol]);
        logger.debug("Old_size: " + old_size);

        /* Figure out the size of the new column-value and set it. */
        int new_size = old_size;
        if (columnType.getBaseType() == SQLDataType.VARCHAR) {
            new_size = value.toString().getBytes().length;
        }

        /* If the previous value was null, we need to reevaluate
         * its offset.  Find the next non-NULL value, and subtract new_size.
         */
        int newOffset = 0;
        if (valueOffsets[iCol] == NULL_OFFSET) {
            logger.debug("Found NULL_OFFSET; iCol=" + iCol);
            int j = iCol + 1;
            while (j < schema.numColumns()) {
                if (valueOffsets[j] == NULL_OFFSET) {
                    j += 1;
                }
                else {
                    ColumnType columnJType = schema.getColumnInfo(j).getType();
                    newOffset = valueOffsets[j] - new_size;
                    logger.debug("Found next offset at index " + j + " with offset " 
                        + valueOffsets[j] + ", size " + getColumnValueSize(columnJType, valueOffsets[j])
                        + " and new offset " + newOffset);
                    break;
                }
            }
            if (j == schema.numColumns()) {
                newOffset = getEndOffset() - new_size;
                logger.debug("Did not find prev offset, so new offset set to " 
                    + newOffset);
            }
            valueOffsets[iCol] = newOffset;
        }

        /* Make room for the new data if necessary. */
        int diff = newOffset - oldOffset;
        if (diff > 0) {
            int moving = valueOffsets[iCol];
            logger.debug("Making room; offset: " + moving + 
                ", diff: " + diff);
            insertTupleDataRange(moving, diff);
        }

        /* Update previous offsets with the new value size. */
         ...
         */
        writeNonNullValue(getDBPage(), valueOffsets[iCol], columnType, value);
    }
